...
func InitObservable() {
	...
	*/
	...
	ObservableMsg = ObservableMsg.Filter(func(item interface{}) bool {
		swear_file, err := os.Open("swear_word.txt")
		if err != nil {
			return false
		}
		defer swear_file.Close()
		scanner := bufio.NewScanner(swear_file)
		for scanner.Scan() {
			word := scanner.Text()
			if strings.Contains(item.(string), word){
				return false
			}
		}
		return true
	...
	ObservableMsg = ObservableMsg.Map(func(_ context.Context, item interface{}) (interface{}, error) {
		str := item.(string)
		sensitive_file, err := os.Open("sensitive_name.txt")
		if err != nil {
			return str, err
		}
		defer sensitive_file.Close()
		scanner := bufio.NewScanner(sensitive_file)
		for scanner.Scan() {
			name := scanner.Text()
			strrune := []rune(name)
			strrune[1] = '*'
			str = strings.Replace(str, name, string(strrune), -1)
		}
		return str, nil
	})
}
